...
namespace Haketon {
		...
		: m_FilePath(filepath)
	{
...

		{
			...
			CreateProgram();
			HK_CORE_WARN("Shader creation took {0} ms", timer.ElapsedMillis());
		}
	...
	std::string OpenGLShader::ReadFile(const std::string& filepath)
	{
		...
		if (in)
		{
			...
			{
				HK_CORE_ERROR("Could not read from file '{0}'", filepath);
			}
		}
		else
		{
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			HK_CORE_ASSERT(Utils::ShaderTypeFromString(type), "Invalid shader type specified");
	...
	void OpenGLShader::Reflect(GLenum stage, const std::vector<uint32_t>& shaderData)
	{
		...
		for (const auto& resource : resources.uniform_buffers)
		{
			...
			HK_CORE_TRACE("  {0}", resource.name);
			HK_CORE_TRACE("    Size = {0}", bufferSize);
			HK_CORE_TRACE("    Binding = {0}", binding);
			HK_CORE_TRACE("    Members = {0}", memberCount);
		}
	}
	...
	void OpenGLShader::SetFloat2(const std::string& name, const FVec2& value)
	{
		GLint location = glGetUniformLocation(m_RendererID, name.c_str());
	...
	void OpenGLShader::SetFloat4(const std::string& name, const FVec4& value)
	{
		GLint location = glGetUniformLocation(m_RendererID, name.c_str());
